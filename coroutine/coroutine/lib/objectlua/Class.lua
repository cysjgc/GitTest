...
function attrReader(self, ...)
	for i = 1, _G.select("#", ...) do
		local attr = _G.select(i, ...)
		self["get" .. _G.string.caps(attr)] = function(self)
			return self[attr]
		end
	end
end

function attrAccessor(self, ...)
	attrReader(self, ...)
	
	for i = 1, _G.select("#", ...) do
		local attr = _G.select(i, ...)
		self["set" .. _G.string.caps(attr)] = function(self, val)
			self[attr] = val
		end
	end
end

-- @objectlua-patch : Mixin
function include(self, mixin)
	local temp = mixin:clone()
	temp['class'		] = nil
	temp['new'			] = nil
	temp['initialize'	] = nil
	temp['dispose'		] = nil
	...
	for k, v in _G.pairs(temp) do
		self[k] = v
	end
	
	if mixin.new then
		self.class.new = mixin.new
	end
end
